 ...
 * @author Timo Salm
 */
class DirectConnectionWavefrontServiceTest {
	...
	void testSendMetricInWavefrontFormat() {
		...
		final String wavefrontDomain = "testWavefrontDomain";
		final String wavefrontToken = "testWavefrontToken";

		final WavefrontService service = new DirectConnectionWavefrontService(restTemplateBuilderMock,
				wavefrontDomain, wavefrontToken);
		...
		verify(restTemplateMock, Mockito.times(1))
				.exchange(eq(wavefrontDomain + "/report"), eq(HttpMethod.POST), argument.capture(),
						eq(Void.class));
		assertThat(Objects.requireNonNull(argument.getValue().getHeaders().get("Authorization")).get(0))
				.isEqualTo("Bearer " + wavefrontToken);
