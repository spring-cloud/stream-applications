...
Then implement your own `java.util.Consumer<ChangeEvent>` (or `ChangeConsumer<ChangeEvent>`) handler and wire it into a DebeziumEngine instance. For example:
...
[TIP]
====
Neither the `Consumer`/`ChangeConsumer` nor the `DebeziumEngine` need to be implemented as `@Bean`! It is just enough to have a `DebeziumEngine.Builder`, set some consumer into it, build an engine and run it from within some `@Service` implementation. Here are is an oversimplified implementation:
[source, java]
----
public void oversimplified(Builder<ChangeEvent<byte[], byte[]>> debeziumBuilder) {
	Executors.newSingleThreadExecutor().execute(debeziumBuilder
			.notifying(changeEvent -> {
				System.out.println(changeEvent.value());
			}).build());
}
----
For real life applications you have to handle the lifecycles of the Debezium Engine and its Executor.
The Spring `Lifecycle` and `SmartLifecycle` interfaces can help manage this.
====
