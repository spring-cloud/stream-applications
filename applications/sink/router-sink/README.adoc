...
$$router.default-output-channel$$:: $$Where to send un-routable messages.$$ *($$String$$, default: `$$<none>$$`)*
...
$$router.refresh-delay$$:: $$How often to check for script changes in ms (if present); < 0 means don't refresh.$$ *($$Integer$$, default: `$$<none>$$`)*
$$router.resolution-required$$:: $$Whether or not channel resolution is required.$$ *($$Boolean$$, default: `$$<none>$$`)*
